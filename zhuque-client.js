  ...
  function init(userConfig) {
    ...
    var observe = new PerformanceObserver(function(list, obs) {
      ...
      for (var i=0; i < perfEntries.length; i++) {
        _listener(perfEntries[i]);
      }
    });
      // subscribe to all performance event types
      observe.observe({entryTypes: userConfig.entryTypes || ['frame', 'mark', 'measure', 'navigation', 'resource', 'server']});
    }
    // NOTE: Private
  function _listener(entry) {
    if(setConfig.remoteLogging) {
      _remoteLogging(entry, setConfig.remoteSettings);
    } else if (console.log) {
